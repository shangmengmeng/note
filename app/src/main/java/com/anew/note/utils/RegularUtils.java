...
public class RegularUtils {
    //数字
    private static final String REGEX_NM = "^[1-9]\\d*|0$";
    //验证手机号
    ...
    public static boolean isNumber(String string){
        return isMatch(REGEX_NM,string);
    }
    /**
     * @param string 待验证文本
     * @return 是否符合手机号格式
    ...
    public static boolean isIdCard(String text) {
        ...
        String reg1 = "[0-9]{15}";
        String regex = "[0-9]{18}";
        return text.toLowerCase().matches(regx) || text.toLowerCase().matches(reg1)
                || text.toLowerCase().matches(regex);
    }
}
